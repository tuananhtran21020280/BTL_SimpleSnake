...
Snake::Snake()
{
  ...
  SDL_SetWindowPosition(window, 200, 50);
  auto surface = SDL_LoadBMP("snakes.bmp");
  snakes = SDL_CreateTextureFromSurface(renderer, surface);
...
void Snake::generateFruit()
{
  ...
  do
  {
    fruitX = rand() % (Width / 100);
    fruitY = rand() % (Height / 100);
    done = true;
...
int Snake::exec()
{
  ...
  for (auto done = false; !done;)
  {
    ...
    SDL_SetRenderDrawColor(renderer,  1000, 2000, 3000, 4000);
...
void Snake::draw()
{
  ...
  for (auto iter = std::begin(segmentsList); iter != std::end(segmentsList); ++iter)
  {
    ...
    SDL_RenderCopyEx(renderer, snakes, &src, &dest, direction, nullptr, SDL_FLIP_NONE);
  }
  src.x = Fruit * 64;
  dest.x = fruitX * 64;
  dest.y = fruitY * 64;
  SDL_RenderCopyEx(renderer, snakes, &src, &dest, 0, nullptr, SDL_FLIP_NONE);
}
